...
"""
Benchmark HISA prefill (block_sparse_mqa_attn_return_logits_kernel) baseline.

Uses npu_lightning_indexer with TND layout as the baseline implementation.
The HISA prefill kernel computes sparse logits for index-based sparse attention.
...
PREFILL_CASES = [
    PrefillCase("prefill_kv128k_q1k", 1024, 131072, "128K KV, q=1K"),
    PrefillCase("prefill_kv16k_q16k", 16384, 16384, "16K prefill, 128 effective blocks"),
    PrefillCase("prefill_kv64k_q64k", 65536, 65536, "64K prefill, 512 effective blocks"),
    PrefillCase("prefill_kv128k_q128k", 131072, 131072, "128K prefill, 1024 effective blocks"),
]


def build_prefill_inputs(case: PrefillCase, device: str):
    Q = case.q_len
    K = case.kv_len
    query = torch.randn(Q, INDEX_N_HEADS, INDEX_HEAD_DIM, dtype=torch.float16, device=device)
    key = torch.randn(K, NUM_HEADS_KV, INDEX_HEAD_DIM, dtype=torch.float16, device=device)
    weights = torch.randn(Q, INDEX_N_HEADS, dtype=torch.float16, device=device)
    actual_seq_lengths_query = torch.tensor([Q], dtype=torch.int32, device=device)
    actual_seq_lengths_key = torch.tensor([K], dtype=torch.int32, device=device)
    return query, key, weights, actual_seq_lengths_query, actual_seq_lengths_key


def run_prefill(query, key, weights, actual_seq_lengths_query, actual_seq_lengths_key):
...
def run_worker(args: argparse.Namespace) -> None:
    ...
    cases = PREFILL_CASES
    if args.case:
        cases = [c for c in PREFILL_CASES if c.name == args.case]
        if not cases:
            raise ValueError(f"Unknown prefill case: {args.case}")

    for case in cases:
        query, key, weights, asq, ask = build_prefill_inputs(case, device)

        for _ in range(args.warmup):
            run_prefill(query, key, weights, asq, ask)
...
MSPROF_OUTPUT_ROOT = Path("/tmp/msprof_hisa_prefill")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HISA prefill benchmark (msprof)")
...
def main() -> None:
    ...
    if args.case:
        ...
        print(f"Available: {[c.name for c in PREFILL_CASES]}")
        return
